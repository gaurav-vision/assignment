...
data = genfromtxt('iris.data.train', delimiter=',', converters={4: lambda x: 1.0 if x == condition else -1.0}, usecols=cols)
...
def applysvm(selection_metric, test):
    ...
    pl.figure('Using selection method: ' + selection_metric)
    ax = pl.subplot(111, projection='3d')
    data = genfromtxt('iris.data.test', delimiter=',', converters={4: lambda x: 1.0 if x == condition else -1.0}, usecols=cols)
